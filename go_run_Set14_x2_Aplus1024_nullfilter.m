...
filter_size = 5;
filter_skip = 4;
fprintf('\nThe number of filters employed is %d, each of size %dx%d.\n',num_filters,filter_size,filter_size);
fprintf('The filters selected are %d cells apart.\n',filter_skip);
fprintf('\n\n');
...
for d=10    %1024
    ...
    mat_file = ['conf_Zeyde_' num2str(dict_sizes(d)) '_finalx' num2str(upscaling) '_' num2str(num_filters) 'filter_' num2str(filter_size) '_skip' num2str(filter_skip)];    
    
    if exist(['matfiles/' mat_file '.mat'],'file')
        disp(['Load trained dictionary...' mat_file]);
        load(['matfiles/' mat_file], 'conf');
    else                            
...
%         conf.interpolate_kernel = 'bicubic';
        conf.filters = load('matfiles/filters.mat');
        conf.filters = conf.filters.filters;
        conf.filters = conf.filters(linspace(1,numel(conf.filters),filter_skip));
        conf.interpolate_kernel = 'bicubic';
    ...
    save(['matfiles/' tag '_' mat_file '_ANR_projections_imgscale_' num2str(imgscale) '_' num2str(filter_size) '_skip' num2str(filter_skip)],'conf');
    
    %% A+ computing the regressors
    Aplus_PPs = [];
        
    fname = ['matfiles/Aplus_x' num2str(upscaling) '_' num2str(dict_sizes(d)) 'atoms' num2str(clusterszA) 'nn_5mil_' num2str(num_filters) 'filter_' num2str(filter_size) '_skip' num2str(filter_skip) '.mat'];
...
%Aplus_PPs{i} = Hi*(inv(Lo*Lo'+llambda*eye(size(Lo,1)))*Lo)'; 
    ...
    fname = ['matfiles/Aplus_x' num2str(upscaling) '_' num2str(dict_sizes(d)) 'atoms' num2str(clusterszA) 'nn_05mil_' num2str(num_filters) 'filter_' num2str(filter_size) '_skip' num2str(filter_skip) '.mat'];    
...
%     end
    %%    
    conf.result_dirImages = qmkdir([input_dir '/results_' tag '_' datestr(now, 'YYYY-mm-dd_HH-MM-SS')]);
    conf.result_dirImagesRGB = qmkdir([input_dir '/results_' tag 'RGB_' datestr(now, 'YYYY-mm-dd_HH-MM-SS')]);
    ...
    %%    
    save(['matfiles/' tag '_' mat_file '_results_imgscale_' num2str(imgscale) '_' num2str(num_filters) 'filters_' num2str(filter_size)],'conf','scores');
%     zip(['zipfiles/' conf.result_dir],
    for i=1:15
        beep
        pause(0.5)
    end
